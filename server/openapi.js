...
function specOpenApi() {
  return {
    ...
    chemins: {
      ...
      'PUT /api/referentiels/articles/{code}/photos': op('Téléversement photo/fiche technique (corps binaire, X-Nom-Fichier)', 'acheteur'),
      'GET /api/referentiels/suggestion-position': op('Positions tarifaires proches d’un libellé (paramètre libelle)', 'lecture'),
      'GET /api/referentiels/articles-doublons': op('Doublons de codes barres et libellés proches', 'lecture'),
      ...
      'GET /api/dossiers/{id}/ecart-liquidation': op('Écart liquidation simulée vs montants retenus (F-M2-05)', 'lecture'),
      'GET /api/dossiers/{id}/comparaison-cles': op('Enjeu des clés multiples face à une clé unique (F-M5-11)', 'lecture'),
      'POST /api/dossiers/{id}/simulation-variation': op('{fret_pct, change_pct, droits_pct} : incidence sur le coût (F-M5-12)', 'lecture'),
